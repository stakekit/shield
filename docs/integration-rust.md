...
Shield can be called from Rust as a subprocess. No Node.js runtime required when using the standalone binary.
...
cargo new shield_example
cd shield_example

# Add dependencies to Cargo.toml, copy the code to src/main.rs
# Copy the shield binary to the project root
